...
#AutoIt3Wrapper_Outfile_x64=Paiamr.exe
#AutoIt3Wrapper_Res_Comment=Paiamr
#AutoIt3Wrapper_Res_CompanyName=Paiamr
#AutoIt3Wrapper_Res_Description=Paiamr
#AutoIt3Wrapper_Res_Fileversion=1.0.0
#AutoIt3Wrapper_Res_LegalCopyright=Paiamr 2025
#AutoIt3Wrapper_Res_LegalTradeMarks=Paiamr 2025
#AutoIt3Wrapper_Res_ProductName=Paiamr
...
Global $g_AppWndTitle = "Paiamr v" & $g_Version
Global $g_AppVersion = "Open Source Release" & @CRLF & "Created by BlueDragon with <3"
	...
	Select
		...
		Case $idMsg = $idBtnCure
			...
			If $bFoundAcro32 = True Then
				MsgBox($MB_SYSTEMMODAL, "Information", "Paiamr does not patch the x32 bit version of Acrobat. Please use the x64 bit version of Acrobat.")
				LogWrite(1, "Paiamr does not patch the x32 bit version of Acrobat. Please use the x64 bit version of Acrobat.")
			EndIf
			If $bFoundGenericARM = True Then
				MsgBox($MB_SYSTEMMODAL, "Information", "This Paiamr build does not support ARM binaries, only x64.")
				LogWrite(1, "This Paiamr build does not support ARM binaries, only x64.")
			EndIf
		...
		Case $idMsg = $idBtnRestore
			GUICtrlSetData($idLog, "Activity Log" & @CRLF & "- - - - - - - - - - -" & @CRLF & @CRLF & "Paiamr Version: " & $g_Version & "" & @CRLF & "Config Version: " & $ConfigVerVar & "" & @CRLF)
			ToggleLog(0)
			GUICtrlSetState($idListview, 128)
			GUICtrlSetState($idBtnDeselectAll, 128)
			GUICtrlSetState($idButtonSearch, 128)
		...
		Case $idMsg = $idBtnRuntimeInfo
			ShowInfoPopup("Select apps may pack the RuntimeInstaller.dll with UPX causing patching to fail. Paiamr can unpack these files so they can then be patched." & @CRLF & @CRLF & @CRLF & @CRLF & _
...
Func MainGui()
	...
	$g_idHyperlinkMain = GUICtrlCreateLabel("paiamr.bluedragon.dev", (595 - 160) / 2, 483, 160, 24, $SS_CENTER)
	...
	$g_idHyperlinkOptions = GUICtrlCreateLabel("paiamr.bluedragon.dev", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	GUICtrlSetTip(-1, "Enable/Disable all Paiamr firewall rules")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
	$idBtnRemoveFW = GUICtrlCreateButton("Remove Rules", 300, 140, 140, 30)
	GUICtrlSetTip(-1, "Remove all Paiamr firewall rules")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
	...
	GUICtrlSetTip(-1, "Remove hosts added by Paiamr")
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)
	...
	$g_idHyperlinkPopup = GUICtrlCreateLabel("paiamr.bluedragon.dev", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$g_idHyperlinkLog = GUICtrlCreateLabel("paiamr.bluedragon.dev", (595 - 160) / 2, 483, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
...
Func FillListViewWithInfo()
	...
	_GUICtrlListView_AddSubItem($idListview, 0, "", 1)
	_GUICtrlListView_AddSubItem($idListview, 1, "Paiamr", 1)
	_GUICtrlListView_AddSubItem($idListview, 2, "Created by BlueDragon with <3", 1)
...
Func MyGlobalPatternPatch($MyFileToPatch, $MyArrayToPatch)
	...
	If $iRows > 0 Then
		...
		LogWrite(1, "File patched by Paiamr " & $g_Version & " + config " & $ConfigVerVar)
		If $bEnableMD5 = 1 Then
			_Crypt_Startup()
...
Func hL_WM_COMMAND($hWnd, $iMsg, $wParam, $lParam)
	...
	If $iIDFrom = $g_idHyperlinkLog Or $iIDFrom = $g_idHyperlinkOptions Or $iIDFrom = $g_idHyperlinkPopup Then
...
Func EnableAllFWRules()
	MemoWrite("Enabling all Paiamr firewall rules...")
	LogWrite(1, "Starting process to enable all Paiamr firewall rules.")
	...
	If $iRuleCount = 0 Then
		MemoWrite("No Paiamr firewall rules found to enable.")
		LogWrite(1, "No Paiamr firewall rules found.")
		LogWrite(1, "Enable rules process completed." & @CRLF)
		ToggleLog(1)
		Return
	EndIf
	...
	Else
		MemoWrite("All Paiamr firewall rules enabled successfully.")
		LogWrite(1, "All Paiamr firewall rules enabled successfully.")
	EndIf
...
Func DisableAllFWRules()
	MemoWrite("Disabling all Paiamr firewall rules...")
	LogWrite(1, "Starting process to disable all Paiamr firewall rules.")
	...
	If $iRuleCount = 0 Then
		MemoWrite("No Paiamr firewall rules found to disable.")
		LogWrite(1, "No Paiamr firewall rules found.")
		LogWrite(1, "Disable rules process completed." & @CRLF)
		ToggleLog(1)
		Return
	...
	Else
		MemoWrite("All Paiamr firewall rules disabled successfully.")
		LogWrite(1, "All Paiamr firewall rules disabled successfully.")
	EndIf
